		...
		stationData.stream().forEach(station -> {
			stationEntities.add(
				RawStationRecord.builder()
					...
					.battalionHq(Boolean.parseBoolean(station.get(3).toString()))
					.emsHq(Boolean.parseBoolean(station.get(4).toString()))
					.opsHq(Boolean.parseBoolean(station.get(5).toString()))
					.latitude(station.get(6))
					.longitude(station.get(7))
					.stationName(station.get(8))
					.streetAddress(station.get(9))
					.city(station.get(10))
					.state(station.get(11))
					.zipCode(station.get(12))
					.phoneNumber(station.get(13))
					.fireBoxes(station.get(14).toString())
					.volunteer(Boolean.parseBoolean(station.get(15).toString()))
					.density(station.get(16))
					.areaDescription(station.get(17))
					.fireHazardDescription(station.get(18))
					.nonFireHazardDescription(station.get(19))
					.build());
		});
